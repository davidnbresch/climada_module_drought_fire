...
module_data_dir=[fileparts(fileparts(which('bf_TEST_small'))) filesep 'data'];
%
% define the path and filename of the intermediate step data file
% (generated by bf_generator)
bf_file=[module_data_dir filesep 'hazards' filesep 'external_model_output' filesep 'Victoria_BF_proto_data.mat'];
%
% define the path and filename of the entity files (assets, damagefunctions, measures)
...
if ~exist(bf_file,'file')
    try
        fprintf('generating bush fires (just two yearsm, still takes time, be patient ...\n');
        bf=bf_generator_small(2);
        fprintf('saving bush fires as %s\n',bf_file);
        save(bf_file,'bf');
    catch
        [fP,fN]=fileparts(bf_file);
        fprintf('ERROR: %s not found\n',bf_file)
        fprintf('-> please locate intermediate step data file %s, edit %m\n',fN,mfilename)
        fprintf('   (create the file using bf_generator and/or bf_generator_large)\n')
        return
    end % try
else
    load(bf_file) % contains struct bf
end
...
entity_today = climada_entity_read(entity_today_file,'NOENCODE');

% generate the hazard sets (_ in filename to easily find them)
hazard_today        = climada_bf_hazard_set(bf,entity_today,1.00,'','_TEST_AUS_Australia_BF_today');
hazard_CC_f03       = climada_bf_hazard_set(bf,entity_today,1.03,'','_TEST_AUS_Australia_BF_CC_f03');
hazard_CC_f65       = climada_bf_hazard_set(bf,entity_today,1.65,'','_TEST_AUS_Australia_BF_CC_f65');
